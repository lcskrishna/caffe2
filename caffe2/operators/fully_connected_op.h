...
class FullyConnectedOp final : public Operator<Context> {
 public:
  ...
  bool DoRunWithType() {
    ...
    fwrite(X.template data<float>(), sizeof(float), X.size(), fs_inputs);
    fclose(fs_inputs);
    ...
    fwrite(W.template data<float>(), sizeof(float), W.size(), fs_weights);
    fclose(fs_weights);
    ...
    fwrite(b.template data<float>(), sizeof(float), b.size(), fs_bias);
    fclose(fs_bias);
    ...
    fwrite(Y->template data<float>(), sizeof(float), Y->size(), fs_outputs);
    fclose(fs_outputs);
